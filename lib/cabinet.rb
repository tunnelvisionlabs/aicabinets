...
if defined?(Sketchup) && Sketchup.respond_to?(:require) && Sketchup.respond_to?(:active_model)
  Sketchup.require('aicabinets/generator/legacy/cabinet')
end

module AICabinets
  # Shared cabinet helpers consumed by UI and generator layers.
  module Cabinet
    module_function
    ...
    def double_door_validity(params_mm:, bay_index:)
      ...
      warn("AI Cabinets: double door validity check failed: #{e.message}")
      failure
    end
    ...
    def compute_partition_thickness_mm(partitions, panel_thickness_mm, interior_width)
      ...
      if override && override >= interior_width - EPSILON_MM
        override = nil
      end
